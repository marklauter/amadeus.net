// Amadeus.Net.Tests\Clients\AirlineRoutesClientTests.cs
...
public sealed class AirlineRoutesClientTests
{
    ...
    public async Task ContextTest()
    {
        ...
        var response =
            ...
            .Bind(context => context.AirlineRoutes.Get(
                AirlineRoutesQuery
                .Create("AF")
                .WithMax(3)
                .WithArrivalCountryCode("FR")))
            .InvokeAsync(tkn);

        _ = response.Match(
            Left: error => Assert.Fail($"expected success: {error}"),
            Right: r => Assert.True(r.Locations.Any()));
    }
}